...
export default class favoritesPag extends Component {
    ...
    componentDidMount() {
        ...
        const fetchData = async () => {
            const response = await axios.get('http://localhost:3003/favorite/' + nome)
    ...
    render() {
        ...
        var liCountries = paises.map(function(pais) {
            return (
                <dl>
                    <dt>
                        <Link className = "paises" to= {{pathname: "/country", state: { name: pais }  }}>{pais}</Link>
                    </dt>
                </dl>
            )
        })
        ...
        return (
            <div>
                <h1>Esse são seus países favoritos</h1>
                <dl>
                    <h4>{liCountries}</h4>
                </dl>
